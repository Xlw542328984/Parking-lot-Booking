...
Page({
  ...
  park_date(event){
    const month = event.detail.getMonth() + 1
    console.log(event.detail.getDate());
    const d = event.detail.getFullYear()+'-'+month+'-'+event.detail.getDate()
    console.log(d);
    this.setData({
      dateshow: false,
      park_date: d,
   ...
   if (day < 10) {
    ...
    request({
      ...
      data: {
        ...
        car_id: this.data.current_car_id,
  ...
  onLoad: function (options) {
    ...
    if(app.globalData.current_car_id == null){
      wx.showLoading({
        title: '请选择车辆',
        icon: 'fail',
        duration: 2000
      ...
      this.setData({
        ...
        current_car_id:app.globalData.current_car_id,
        car_number:app.globalData.car_number
      })
    }
  },
